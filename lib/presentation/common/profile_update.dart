...
class _ProfileUpdatePageState extends State<ProfileUpdatePage> {
  ...
  final _walletAddressController = TextEditingController(); // Added wallet controller
  ...
  void dispose() {
    ...
    _walletAddressController.dispose(); // Dispose wallet controller
    super.dispose();
  }
      ...
      if (doc.exists) {
        ...
        if (mounted) {
          setState(() {
            ...
            _walletAddressController.text = data['walletAddress'] ?? ''; // Set wallet address
  ...
  Future<void> _updateProfile() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('users').doc(_userId).update({
        ...
        'walletAddress': _walletAddressController.text.trim(), // Update wallet address
  ...
  String? _validateWalletAddress(String? value) {
    if (value == null || value.isEmpty) {
      return null; // Wallet address is optional
    }
    ...
    if (trimmed.startsWith('0x') && trimmed.length == 42) {
      ...
      if (RegExp(r'^[a-fA-F0-9]+$').hasMatch(hexPart)) {
        return null; // Valid Ethereum address
      }
    }
    
    // Bitcoin address validation (basic check for length and characters)
    if ((trimmed.length >= 26 && trimmed.length <= 35) || 
        (trimmed.length >= 42 && trimmed.length <= 62)) {
      if (RegExp(r'^[a-zA-Z0-9]+$').hasMatch(trimmed)) {
        return null; // Likely valid Bitcoin address
      }
    }
